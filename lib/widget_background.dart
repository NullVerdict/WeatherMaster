...
Future<void> updateHomeWidget(weather, {bool updatedFromHome = false}) async {
  try {
    ...
    final triggerFromWorker =
        PreferencesHelper.getBool(PrefKeys.triggerFromWorker) ?? false;
    String? lastUpdatedString =
        PreferencesHelper.getString(PrefKeys.lastUpdatedFromHome);
    ...
    for (int i = 0; i < 4; i++) {
      ...
      final localeString =
          PreferencesHelper.getString(PrefKeys.locale) ?? 'en';
    ...
    final localeString = PreferencesHelper.getString(PrefKeys.locale) ?? 'en';
